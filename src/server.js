...
server.post("/cadastro", async (request, reply) => {
	...
	if (!user.username || !user.email || !user.password) {
		return reply
			.status(400)
			.send({ error: "Nome de usuário, email e senha são obrigatórios." });
	}
...
server.post("/vehicles", async (request, reply) => {
	...
	if (!vehicle.model || !vehicle.brand || !vehicle.plate || !vehicle.year) {
		return reply
			.status(400)
			.send({ error: "Model, brand, plate e year são obrigatórios." });
	}
	...
	try {
		...
		return reply
			.status(201)
			.send({ message: "Veículo cadastrado com sucesso!" });
	} catch (error) {
		return reply.status(400).send({ error: error.message });
	}
});
...
server.post("/movements", async (request, reply) => {
	...
	if (
		...
		return reply.status(400).send({
			error:
				"Vehicle ID, start location, end location e distance são obrigatórios.",
		});
	}
...
server.delete("/movements/:id", async (request, reply) => {
	const { id } = request.params;
	if (!id) {
		return reply
			.status(400)
			.send({ error: "ID é obrigatório para deletar a movimentação." });
	}
	try {
		const deleted = await database.deleteMovement(id);
		if (!deleted) {
			return reply.status(404).send({ error: "Movimentação não encontrada." });
		}
		return reply
			.status(200)
			.send({ message: "Movimentação deletada com sucesso." });
	} catch (error) {
		return reply.status(500).send({ error: "Erro ao deletar a movimentação." });
	}
});
...
server.delete("/refills/:id", async (request, reply) => {
	const { id } = request.params;

	if (!id) {
		return reply
			.status(400)
			.send({ error: "ID é obrigatório para deletar o abastecimento." });
	}

	try {
		const deleted = await database.deleteRefill(id);

		if (!deleted) {
			return reply.status(404).send({ error: "Abastecimento não encontrado." });
		}

		return reply
			.status(200)
			.send({ message: "Abastecimento deletado com sucesso." });
	} catch (error) {
		return reply
			.status(500)
			.send({ error: "Erro ao deletar o abastecimento." });
	}
});

server.get("/drivers", async (request, reply) => {
	try {
		const drivers = await database.listDrivers();
		return reply.send(drivers);
	} catch (error) {
		return reply.status(500).send({ error: "Erro ao listar motoristas." });
	}
});
...
server.post("/drivers", async (request, reply) => {
	...
	if (!driver.name || !driver.license_number) {
		return reply.status(400).send({
			error: "Nome e número da habilitação são obrigatórios.",
		});
	}

	try {
		await database.createDriver(driver);
		return reply
			.status(201)
			.send({ message: "Motorista cadastrado com sucesso!" });
	} catch (error) {
		return reply.status(400).send({ error: error.message });
	}
});
...
server.delete("/drivers/:id", async (request, reply) => {
	...
	try {
		const deleted = await database.deleteDriver(id);
		if (!deleted) {
		...
		return reply.status(500).send({ error: "Erro ao deletar o motorista." });
	}
});
